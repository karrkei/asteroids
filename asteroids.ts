...
function asteroids() {
  ...
  */
  ...
  let ship_bound = new Elem(svg, 'ellipse',g.elem)
  ...
  let score = 0
    const game_display = document.getElementById("display")!
  ...
  polygons.
  */
  let points = "10,0 13,2 26,1 30,8 29,15 31,20 26,31 22,31 8,29 1,22 2,16 1,7 4,0",
  /*
...
//functions
  ...
  function updatePositionX(positionX: number, speed: number, angle: number):number {
    /*
  ...
  function updatePositionY(positionY: number, speed: number, angle: number):number {
    /*
  ...
  function bulletX(positionX: number, rotation:number):number {
    /*
  ...
  function bulletY(positionY: number, rotation:number):number {
    /*
    this function takes in 2 numbers, positionX (current y position of the bullet) and rotation (direction in which the bullet is supposed to move towards 
    next) and generates the next y position for the bullet. I have created a separate function from updatePosition X because I don't want the bullets to wrap
    around.
  ...
  function removeShotAsteroids(){
    ...
    */
    ...
    let asteroids_remove = asteroids_list.filter(asteroid=>Number(asteroid.getAttribute('shot'))>=1 )
    asteroids_remove.forEach(asteroid=>a.elem.removeChild(asteroid))
    asteroids_list = Array.from(a.elem.children)
  ...
  function checkCollision (asteroid: Element,lives:number) :number{
    /*
    ...
    */
    ...
    let distance = Math.sqrt(Math.pow((shipX - (Number(asteroid.getAttribute('asteroidX'))+(asteroid.getBoundingClientRect().width)/2)),2) 
  ...
  function checkAsteroid(asteroid:Element) : Element[]{
    /*
  ...
  function explodeShip(lives_diff:number) : string{
    /*
  ...
  function checkShot (bullet:Elem, asteroid: Element) : string{
    /*
    ...
    */
  let distance = Math.sqrt(Math.pow((Number(bullet.attr('cx'))- (Number(asteroid.getAttribute('asteroidX'))+(asteroid.getBoundingClientRect().width)/2)),2) 
 ...
 function addScore(shot:number, size: number,score: number) : number{
    /*
  ...
  function clearOld(){
    ...
    */
    ...
    let clearold = Array.from(a.elem.children)

    return clearold.length > 0 ? 
    clearold.filter(asteroid=> Number(asteroid.getAttribute('duration'))>=30 && 
  ...
  function terminateGame(){
    ...
    */
    let asteroids = Array.from(a.elem.children)
    asteroids.forEach(asteroid=>a.elem.removeChild(asteroid))
    game_duration = 0
    game_display.innerHTML =  display(lives)
    
  }
  function endGame(lives:number) {
    /*
  ...
  function display(lives:number) : string{
    /*
  ...
  Observable
    ...
    .subscribe(()=>{ 
      ...
      let coordinates= getRandomNum(0,7),        
      ...
      //creating asteroids
      let asteroids = new Elem(svg, 'polygon', a.elem) 
        .attr("style","fill:white;stroke:black;stroke-width:1")
        .attr("points", points)
      ...
      let lives_diff = lives_before - lives
      ...
      checkAsteroid(asteroid).forEach(asteroid=>{
      ...
      .subscribe(()=>{
        ...
        Array.from(a.elem.children).forEach(asteroid=>{
    ...
    Observable
      ...
      .subscribe(()=>{ 
        let x = endGame(lives)
        let toClear = clearOld()
        toClear.forEach(asteroid=>a.elem.removeChild(asteroid))
        game_display.innerHTML =  display(lives)
  ...
  keydown
    .filter(({keyCode})=> (keyCode === 37 || keyCode == 39) && (lives >0 && lives <= 3))
    .map(({keyCode})=> ({
 ...
 //Observable to move the ship forward when the up key is pressed.
  keydown
    .filter(({keyCode})=> keyCode === 38 && (lives >0 && lives <= 3))
    .subscribe(() => {
      ...
      Observable
        ...
        .subscribe(()=>{
          ...
          asteroids_list.forEach(asteroid=>{            
            ...
            let shot = Number(asteroid.getAttribute('shot')), 
                size = Number(asteroid.getAttribute('size'));
        ...
        Observable
          ...
          .subscribe(()=>{g.elem.removeChild(g.elem.children[2])})
